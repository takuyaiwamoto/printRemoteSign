...
(function(root, factory){
  ...
  function create({ canvas, dpr = (typeof devicePixelRatio!=='undefined'?devicePixelRatio:1), bufferMs = 200, eraserScale = 1.3 }) {
    ...
    function clearAuthor(authorId) {
      ...
      const lay = otherLayers.get(key);
      if (lay) lay.ctx.clearRect(0,0,lay.canvas.width, lay.canvas.height);
      ...
      for (const [id, s] of strokes) {
        if (!s.leadDrawn && s.points.length >= 2) {
          ...
          if (second.time <= target || s.ended) {
            ...
            ctxLead.lineWidth = (s.tool==='eraser'?eraserScale:1.0) * (s.sizeDev || (s.sizeCss * DPR));
            ctxLead.beginPath();
        ...
        let ready = (() => { for (let i = s.points.length - 1; i >= 2; i--) if (s.points[i].time <= target) return i; return 0; })();
        if (s.ended) ready = Math.max(ready, s.points.length - 1);
        if (s.curIndex === undefined) {
          if (ready >= 2) {
            ...
            const ctx = getLayer(String(s.author)).ctx;
            ctx.beginPath(); ctx.fillStyle = s.color;
            const r = (s.sizeDev || (s.sizeCss * DPR))/2;
            ctx.arc(s.points[0].x, s.points[0].y, r, 0, Math.PI*2); ctx.fill();
            ...
            ctx.lineWidth = (s.tool==='eraser'?eraserScale:1.0) * (s.sizeDev || (s.sizeCss * DPR));
        ...
        while (s.curIndex <= ready) {
          ...
          const segLen=Math.hypot(endPoint.x-m1.x,endPoint.y-m1.y)+1e-3;
          ...
          const desired=(i<ready || isFinalSegment)?1:timeT;
          while(s.t<desired-1e-6){
            const nt=Math.min(desired, s.t+dt);
            const np=qPoint(m1,p1,endPoint,nt);
        ...
        if (s.ended && s.curIndex > lastSegment) strokes.delete(id);
      }
    }
